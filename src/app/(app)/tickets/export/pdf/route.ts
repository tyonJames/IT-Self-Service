...
/** `/tickets/export/pdf/` — formatted report with the SLA table (spec §5.17). */
export async function GET(request: NextRequest) {
  try {
    ...
    const buffer = await exportService.ticketsToPdf(filters, period.label);

    await auditService.record({
      event: SecurityEvent.FILE_ACCESS,
      message: "Exported tickets to PDF",
    ...
    const filename = `radx-tickets-${new Date().toISOString().slice(0, 10)}.pdf`;
    ...
    return handleApiError(error, "GET /tickets/export/pdf");
  }
}
